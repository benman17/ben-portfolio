...
import { BarChart3, Kanban, Workflow, ArrowRight } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {SKILL_CATEGORIES.map((pillar, index) => {
            ...
            return (
              <div
                key={pillar.title}
                className="p-6 bg-[#08080c] border border-[#1a1a20] flex flex-col justify-between group hover:border-[#38bdf8] transition-colors"
              >
                <div>
                ...
                {/* Card CTA Link */}
                <div className="pt-4 border-t border-[#1a1a20]">
                  <Link
                    href={index === 1 ? '/project-management' : index === 0 ? '/analytics' : '/projects'}
                    className="inline-flex items-center gap-2 text-xs font-mono font-bold text-white hover:text-[#38bdf8] transition-colors"
                  >
